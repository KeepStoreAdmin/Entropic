...
<asp:Content ID="Content1" ContentPlaceHolderID="cph" Runat="Server">   
    ...
    <script runat="server">
        ...
        Function calcola_risparmio(ByVal prezzo_listino As Double, ByVal prezzo1 As Object, ByVal prezzo2 As Object) As Double
            ...
            If IsDBNull(prezzo2) Then
                If IsDBNull(prezzo1) Then
                    Return 0
                End If
        ...
        Function controlla_prezzo(ByVal prezzo As Double, ByVal prezzo_ivato As Double, ByVal prezzo_promo As Double, ByVal prezzo_promo_ivato As Double, ByVal iva_tipo As Integer) As String
	...
	<script runat="server">
		...
		Function compatta_testo(ByVal testo As String, ByVal lunghezza As Integer) As String
			  Dim testoFinale = Left(testo, lunghezza)
		  ...
		  Function sconto(ByVal listino_ufficiale As Double, ByVal prezzo_promo As Double, ByVal prezzo_promo_ivato As Double, ByVal iva_articolo As Double) As String
			  ...
			  If ((prezzo_promo > 0) And (prezzo_promo_ivato > 0) And (listino_ufficiale > 0)) Then
				  ...
				  Else
					  percentuale = String.Format("{0:0}", (((listino_ufficiale * ((iva_articolo / 100) + 1)) - prezzo_promo_ivato) * 100) / (listino_ufficiale * ((iva_articolo / 100) + 1)))
				  End If
			  End If
			  Return "- " & percentuale & "%"
		  End Function
	...
	<!--<h2>Scelti per te <i class="far fa-thumbs-up"></i></h2>-->
	<div class="container-fluid scelti-per-te bg-white home-box-position mt-3 bg-shadow">
		<h1 style="padding:0px;"><img src="Public/Images/scelti_per_te.png" width="100%" style="max-height: 50px;"></h1>
        ...
        <ItemTemplate>
			...
			<div class="col-12 col-sm-6 col-md-4 mb-4">
				...
				<div class="card" style="padding-bottom: 10px;min-height: 430px;">
					...
					<span style="font-weight:bold;">sconto</span>
					<br />
					<span style="font-weight:bold;"><%#sconto(Eval("ListinoUfficiale"), IIf(IsDBNull(Eval("PrezzoPromo")), 0, Eval("PrezzoPromo")), IIf(IsDBNull(Eval("PrezzoPromoIvato")), 0, Eval("PrezzoPromoIvato")), Eval("iva"))%></span>
					</div>
					...
					<div style="height: 130px;">
						...
						<a href="articolo.aspx?id=<%# Eval("ArticoliId") %>&TCId=<%# Eval("TCId") %>">
							<img class="card-img-top" src='<%# checkImg(Eval("img1")) %>' />
						</a>
									
					</div>
					
					<div class="card-body">
						<h5 class="card-title" style="min-height: 57px;"><%#compatta_testo(Eval("Descrizione1"), 30)%></h5>
						<p class="card-text">
		
							<%#compatta_testo(IIf(Eval("taglia").Equals(DBNull.Value), "", Eval("taglia")) & IIf(Eval("taglia").Equals(DBNull.Value) Or Eval("colore").Equals(DBNull.Value), "", ", ") & IIf(Eval("colore").Equals(DBNull.Value), "", Eval("colore")), 22)%>
														
						</p>
					   
						<div>
							<div>
								<span style="background-image:url('Images/back_menu.png'); background-position:center; background-color:gray; background-repeat:repeat-x; display:block; color:white; font-weight:bold; padding:5px; text-align: center;">IL TUO PREZZO</span>
								<span id="infobar" style="background-image:url('Images/back_menu.png'); background-position:center; background-repeat:repeat-x; display:block; color:white; font-size:11pt; padding:5px;">
									...
									<%If Session("IvaTipo") = 1 Then%>
										<div style="width:100%; height:31px; overflow:hidden; text-align:center; font-weight:bold;" ><%#prezzo_formattato(IIf(Eval("InOfferta") = 0, Eval("Prezzo", "{0:C}"), Eval("prezzoPromo", "{0:C}")))%></div>
									<%Else%>
										<div style="width:100%; height:31px; overflow:hidden; text-align:center; font-weight:bold;" ><%#prezzo_formattato(IIf(Eval("InOfferta") = 0, Eval("PrezzoIvato", "{0:C}"), Eval("prezzoPromoIvato", "{0:C}")))%></div>
									<%End If%>
								</span>
							</div>
							<div class="mt-2" style="font-size: 0.7rem;min-height: 64px;"><%#compatta_testo(Eval("Descrizione2"), 100)%></div>
						</div>
							 
						<div style="margin-top: 10px">Codice: <%#Eval("Codice")%></div>
						 
						 <!-- Listino Ufficiale -->
						 <div style="height: 35px">
					   
							<%If Session("IvaTipo") = 1 Then%>
								<div style="width:100%;/* height:15px;*/ overflow:hidden; text-align:center; font-weight:bold; font-size:9pt; display:<%# controlla_risparmio(Eval("ListinoUfficiale"),Eval("Prezzo"),Eval("prezzoPromo")) %>;"" >prezzo di listino <span style="text-decoration:line-through; color:Red;"><%#Eval("ListinoUfficiale", "{0:C}")%></span></div>
								<div style="width:100%;/* height:20px;*/ overflow:hidden; text-align:center; font-weight:bold; font-size:11pt; display:<%# controlla_risparmio(Eval("ListinoUfficiale"),Eval("Prezzo"),Eval("prezzoPromo")) %>;" >risparmi <span style="color:Red; font-size:9pt;"><%#String.Format("{0:C}", calcola_risparmio(Eval("ListinoUfficiale"), Eval("Prezzo"), Eval("prezzoPromo")))%></span></div>
							<%Else%>
								<div style="width:100%;/* height:15px;*/ overflow:hidden; text-align:center; font-weight:bold; font-size:9pt; display:<%# controlla_risparmio(Eval("ListinoUfficiale") * ((Eval("iva") / 100000) + 1),Eval("PrezzoIvato"),Eval("prezzoPromoIvato")) %>;"" >prezzo di listino <span style="text-decoration:line-through; color:Red;"><%#String.Format("{0:C}", (Eval("ListinoUfficiale") * ((Eval("iva") / 100000) + 1)))%></span></div>
								<div style="width:100%;/* height:20px;*/ overflow:hidden; text-align:center; font-weight:bold; font-size:11pt; display:<%# controlla_risparmio(Eval("ListinoUfficiale") * ((Eval("iva") / 100000) + 1),Eval("PrezzoIvato"),Eval("prezzoPromoIvato")) %>;" >risparmi <span style="color:Red; font-size:9pt;"><%#String.Format("{0:C}", calcola_risparmio(Eval("ListinoUfficiale") * ((Eval("iva") / 100000) + 1), Eval("PrezzoIvato"), Eval("prezzoPromoIvato")))%></span></div>
							<%End If%> 
						</div>
				</div>
    ...
    <asp:SqlDataSource ID="SqlDataSource_Pubblicita_id4_pos1" runat="server" ConnectionString="<%$ ConnectionStrings:EntropicConnectionString %>"
        ProviderName="<%$ ConnectionStrings:EntropicConnectionString.ProviderName %>"
    ...
    <asp:Repeater ID="RepeaterPubblicita_id4_pos1" runat="server" DataSourceID="SqlDataSource_Pubblicita_id4_pos1" EnableViewState="False" >
        <ItemTemplate>
            ...
            <div style="max-width:100%" class="img-max-100">
                <asp:HyperLink ID="hlBanner" runat="server" NavigateUrl='<%# "click.aspx?id=" & Eval("id") & "&link=" & Server.UrlEncode(Eval("link")) %>' EnableViewState="false" tooltip='<%# Eval("Descrizione") %>' ImageUrl='<%# "Public/Banner/" & Eval("img_path") %>' Target='_blank'></asp:HyperLink>
            </div>
        </ItemTemplate>
    </asp:Repeater>
    
    <!-- BANNER PUBBLICITARIO (id_posizione_banner=4)(ordinamento=2) -->
    <asp:SqlDataSource ID="SqlDataSource_Pubblicita_id4_pos2" runat="server" ConnectionString="<%$ ConnectionStrings:EntropicConnectionString %>"
        ProviderName="<%$ ConnectionStrings:EntropicConnectionString.ProviderName %>"
        SelectCommand="SELECT id, id_Azienda, data_inizio_pubblicazione, data_fine_pubblicazione, limite_click, limite_impressioni, id_posizione_banner, numero_click_attuale, numero_impressioni_attuale, link, img_path, titolo, descrizione, abilitato from pubblicitaV2"
        UpdateCommand="update pubblicitaV2 SET numero_impressioni = numero_impressioni + 1 WHERE (id = 1)">
    </asp:SqlDataSource>
    ...
    <asp:SqlDataSource ID="SdsNewArticoli" runat="server" 
        ConnectionString="<%$ ConnectionStrings:EntropicConnectionString %>" 
        ProviderName="<%$ ConnectionStrings:EntropicConnectionString.ProviderName %>" 
        SelectCommand="SELECT * FROM articoli LIMIT1">
    </asp:SqlDataSource>
    ...
    <div class="container-fluid bg-white pb-4 mt-3 home-box-position bg-shadow">
		<h1 style="padding:0px;"><img src="Public/Images/novita.png" width="100%" style="max-height: 50px;"/></h1>
							...
							<div style="padding: 1.5rem">
								...
								<div style="text-align:center;">
									<div style="width:255px; min-height: 160px;margin:auto; overflow:hidden; position:relative;">
										<a href="articolo.aspx?id=<%# Eval("ArticoliId") %>&TCId=<%# Eval("TCId") %>"><img border="0" style="max-width: 180px; max-height: 120px;" src='<%# checkImg(Eval("img1")) %>' alt="" /></a>
										<div style="position:absolute; top:0px; left:44px; z-index:999; width:180px; height:50px; overflow:hidden;">
											...
											<div style="height:50px; float:left; width:50px; margin:0px;  padding-top: 0px; padding-left: 0px;">
												<img src="Public/Images/bollinoSpedizioneVetrina.png" alt="Questo articolo verrà spedito GRATIS !!!" style="display:<%# spedire_gratis(Eval("SpeditoGratis")) %>;" />
											</div>
											<!-- Immagine in offerta -->
											<div style=" width:50px; height:50px; margin:0px 0px; float:right;">
												<img src="Public/Images/bollinoPromoVetrina.png" alt="Articolo in PROMO !!!" style="display:<%# controlla_promo(Eval("inOfferta"))%>;" />
											</div>
										</div>
										<div style="text-align:center;">
											<a href="articolo.aspx?id=<%# Eval("ArticoliId") %>&TCId=<%# Eval("TCId") %>"><img border="0" width="80px" src='<%# "Public/Marche/" & Eval("Marche_img") %>' alt="" /></a>
										</div>
									</div>
								</div>
								<div style="padding-left:25px; max-width:255px; margin:auto; position:relative; text-align:justify; font-size:8pt; font-weight:bold; margin-top: 10px;">
									<div>
										<a href="articolo.aspx?id=<%# Eval("ArticoliId") %>&TCId=<%# Eval("TCId") %>"><%#adatta_testo(Eval("Descrizione1"), 80)%><br/><%#compatta_testo(IIf(Eval("taglia").Equals(DBNull.Value), "", Eval("taglia")) & IIf(Eval("taglia").Equals(DBNull.Value) Or Eval("colore").Equals(DBNull.Value), "", ", ") & IIf(Eval("colore").Equals(DBNull.Value), "", Eval("colore")), 22)%> </a>
									</div>
									<span style="background-image:url('Images/back_menu.png'); background-position:center; background-color:gray; background-repeat:repeat-x; display:block; color:white; font-weight:bold; padding:5px; text-align:center; font-size:9pt;">IL TUO PREZZO</span>
									<span id="infobar" style="background-image:url('Images/back_menu.png'); background-position:center; background-repeat:repeat-x; display:block; color:white; font-size:11pt; padding:5px; text-align:center;">
										<%#controlla_prezzo(IIf(IsDBNull(Eval("prezzo")), 0, Eval("prezzo")), IIf(IsDBNull(Eval("prezzoIvato")), 0, Eval("prezzoIvato")), IIf(Eval("InOfferta") = 0, 0, Eval("prezzoPromo")), IIf(Eval("InOfferta") = 0, 0, Eval("PrezzoPromoIvato")), Session("IvaTipo"))%>
									</span>
								</div>
							</div>
						</div>
								...
								<div style="width:80px; height:90px; float:left; overflow:hidden; position:relative;">
									<div style="position:absolute; top:0px; left:0px; z-index:999; width:80px; height:50px; overflow:hidden;">
										...
										<div style="height:40px; float:left; width:40px; margin:0px;  padding-top: 0px; padding-left: 0px;">
											<img src="Public/Images/bollinoSpedizioneVetrina.png" alt="Questo articolo verrà spedito GRATIS !!!" style="display:<%# spedire_gratis(Eval("SpeditoGratis")) %>; width:40px;" />
										</div>
										<!-- Immagine in offerta -->
										<div style=" width:40px; height:40px; margin:0px 0px; float:right;">
											<img src="Public/Images/bollinoPromoVetrina.png" alt="Articolo in PROMO !!!" style="display:<%# controlla_promo(Eval("inOfferta"))%>; width:40px;" />
										</div>
									</div>
									<a href="articolo.aspx?id=<%# Eval("ArticoliId") %>&TCId=<%# Eval("TCId") %>"><img border="0" width="85px" src='<%# checkImg(Eval("img1")) %>' /></a>
								</div>
								<div style="width:215px; height:90px; float:right; position:relative; font-size:7pt; padding-left:5px; text-align:justify;">
									<a href="articolo.aspx?id=<%# Eval("ArticoliId") %>&TCId=<%# Eval("TCId") %>"><%#adatta_testo(Eval("Descrizione1"), 100)%><br/><%#compatta_testo(IIf(Eval("taglia").Equals(DBNull.Value), "", Eval("taglia")) & IIf(Eval("taglia").Equals(DBNull.Value) Or Eval("colore").Equals(DBNull.Value), "", ", ") & IIf(Eval("colore").Equals(DBNull.Value), "", Eval("colore")), 22)%></a>
									<div style="position:absolute; bottom:3px; left:5px; height:25px; text-align:left; /*padding-top:5px*/; font-size:6pt;"><%#"<b>COD.</b><br>" & Eval("Codice")%></div>
									<div style="position:absolute; bottom:-8px; right:3px; width:100px; height:30px; text-align:right; /*padding-top:5px*/; font-weight:bold; font-size:12pt; color:white;"><%#controlla_prezzo(IIf(IsDBNull(Eval("prezzo")), 0, Eval("prezzo")), IIf(IsDBNull(Eval("prezzoIvato")), 0, Eval("prezzoIvato")), IIf(Eval("InOfferta") = 0, 0, Eval("prezzoPromo")), IIf(Eval("InOfferta") = 0, 0, Eval("PrezzoPromoIvato")), Session("IvaTipo"))%></div>
								</div>
							</div>
	...
	<h1 style="padding:0px;"><img src="Public/Images/piu_venduti.png" alt="" width="100%" style="max-height: 50px;" /></h1>
	<div class="row mx-1">
								...
								<tr>
									<td>
										<div style="color:black; font-weight:bold; font-size:6pt; text-align:left; margin: 5px; min-height: 36px;">
											<%#compatta_testo(Eval("Descrizione1"), 22)%>
											<br/><%#compatta_testo(IIf(Eval("taglia").Equals(DBNull.Value), "", Eval("taglia")) & IIf(Eval("taglia").Equals(DBNull.Value) Or Eval("colore").Equals(DBNull.Value), "", ", ") & IIf(Eval("colore").Equals(DBNull.Value), "", Eval("colore")), 22)%>
										</div>
										<div style="text-align:center; background-color:#ebebeb;">
											<span style="background-image:url('Images/back_menu.png'); background-position:center; background-color:gray; background-repeat:repeat-x; display:block; color:white; font-weight:bold; padding-top:2px; padding-bottom:2px; font-size:8pt;">IL TUO PREZZO</span>
    ...
    <div id="boxpromo">
		<div class="container-fluid mb-3 partners mt-3 bg-shadow home-box-position">
			<div class="row">
				<h2 style="font-size: 1.7rem;">Rivenditori Ufficiali - I nostri brand  <i class="fa fa-handshake"></i></h2>
			</div>	
			<div class="row">
					...
					<ItemTemplate>
						<div class="col-3 col-md-2 align-self-center py-2">
							...
							<a href='<%# "articoli.aspx?ct=30000&mr=" & Eval("id") %>'>
								<img src="<%# "Public/Marche/" & Eval("img") %>" style="width:100%; max-width:150px;" alt="<%# Eval("Descrizione") %>" title="<%# Eval("Descrizione")%>" />
							</a>
						</div>
					</ItemTemplate>
				</asp:Repeater>
    ...
    <br /><asp:Label ID="lblPrezzi" runat="server" Text="*Prezzi" Font-Size="7pt" Font-Names="arial"></asp:Label><br />
