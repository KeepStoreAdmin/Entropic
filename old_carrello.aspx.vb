...
Partial Class carrello
    ...
    Sub preleva_prezzi_articoli()
        ...
        If LoginId = 0 Then
            cmd.CommandText = "UPDATE carrello  INNER JOIN  (  SELECT carrello.id AS idCarrello, carrello.ArticoliId, vsuperarticoli.id, vsuperarticoli.Nlistino, vsuperarticoli.InOfferta, vsuperarticoli.DescrizioneIvaRC, IF((InOfferta=1) AND ((CDate(OfferteDataInizio)<=CURDATE()) AND (CDate(OfferteDataFine)>=CURDATE())),vsuperarticoli.PrezzoPromo,vsuperarticoli.Prezzo) AS new_Prezzo, IF((InOfferta=1) AND ((CDate(OfferteDataInizio)<=CURDATE()) AND (CDate(OfferteDataFine)>=CURDATE())),IF(" & Session("Iva_Utente") & ">-1,((vsuperarticoli.PrezzoPromo)*((" & Session("Iva_Utente") & "/100)+1)),vsuperarticoli.PrezzoPromoIvato),IF(" & Session("Iva_Utente") & ">-1,((vsuperarticoli.Prezzo)*((" & Session("Iva_Utente") & "/100)+1)),vsuperarticoli.PrezzoIvato)) AS new_PrezzoIvato, IF((InOfferta=1) AND ((CDate(OfferteDataInizio)<=CURDATE()) AND (CDate(OfferteDataFine)>=CURDATE())),IF(" & Session("IvaReverseCharge_Utente") & ">-1,((vsuperarticoli.PrezzoPromo)*((" & Session("IvaReverseCharge_Utente") & "/100)+1)),vsuperarticoli.PrezzoPromoIvato),IF(" & Session("IvaReverseCharge_Utente") & ">-1,((vsuperarticoli.Prezzo)*((" & Session("IvaReverseCharge_Utente") & "/100)+1)),-1)) AS new_PrezzoRC FROM carrello  INNER JOIN vsuperarticoli  ON (carrello.ArticoliId=vsuperarticoli.id)  WHERE (vsuperarticoli.Nlistino=" & Session("Listino") & ") AND (SessionID='" & Me.Session.SessionID & "')) AS t1  ON idCarrello=carrello.id SET carrello.Prezzo=new_Prezzo, carrello.PrezzoIvato=new_PrezzoIvato, carrello.ValoreIvaRC=new_PrezzoRC, carrello.DescrizioneIvaRC=DescrizioneIvaRC"
        Else
            cmd.CommandText = "UPDATE carrello  INNER JOIN  (  SELECT carrello.id AS idCarrello, carrello.ArticoliId, vsuperarticoli.id, vsuperarticoli.Nlistino, vsuperarticoli.InOfferta, vsuperarticoli.DescrizioneIvaRC, IF((InOfferta=1) AND ((CDate(OfferteDataInizio)<=CURDATE()) AND (CDate(OfferteDataFine)>=CURDATE())),vsuperarticoli.PrezzoPromo,vsuperarticoli.Prezzo) AS new_Prezzo, IF((InOfferta=1) AND ((CDate(OfferteDataInizio)<=CURDATE()) AND (CDate(OfferteDataFine)>=CURDATE())),IF(" & Session("Iva_Utente") & ">-1,((vsuperarticoli.PrezzoPromo)*((" & Session("Iva_Utente") & "/100)+1)),vsuperarticoli.PrezzoPromoIvato),IF(" & Session("Iva_Utente") & ">-1,((vsuperarticoli.Prezzo)*((" & Session("Iva_Utente") & "/100)+1)),vsuperarticoli.PrezzoIvato)) AS new_PrezzoIvato, IF((InOfferta=1) AND ((CDate(OfferteDataInizio)<=CURDATE()) AND (CDate(OfferteDataFine)>=CURDATE())),IF(" & Session("IvaReverseCharge_Utente") & ">-1,((vsuperarticoli.PrezzoPromo)*((" & Session("IvaReverseCharge_Utente") & "/100)+1)),vsuperarticoli.PrezzoPromoIvato),IF(" & Session("IvaReverseCharge_Utente") & ">-1,((vsuperarticoli.Prezzo)*((" & Session("IvaReverseCharge_Utente") & "/100)+1)),-1)) AS new_PrezzoRC FROM carrello  INNER JOIN vsuperarticoli  ON (carrello.ArticoliId=vsuperarticoli.id)  WHERE (vsuperarticoli.Nlistino=" & Session("Listino") & ") AND (LoginId='" & LoginId & "')) AS t1  ON idCarrello=carrello.id SET carrello.Prezzo=new_Prezzo, carrello.PrezzoIvato=new_PrezzoIvato, carrello.ValoreIvaRC=new_PrezzoRC, carrello.DescrizioneIvaRC=DescrizioneIvaRC"
        End If
        cmd.ExecuteNonQuery()
    ...
    Protected Sub Page_PreRender(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.PreRender
        ...
        If LoginId = 0 Then
            Me.sdsArticoli.SelectCommand = "SELECT vcarrello.*, articoli.SpedizioneGratis_Listini, articoli.SpedizioneGratis_Data_Inizio, articoli.SpedizioneGratis_Data_Fine FROM vcarrello LEFT OUTER JOIN articoli ON vcarrello.ArticoliId = articoli.id WHERE ((SessionID='" & SessionID & "') AND (articoli.SpedizioneGratis_Listini Is NULL)) OR ((SessionID='" & SessionID & "') AND (articoli.SpedizioneGratis_Listini Is Not NULL) AND (SpedizioneGratis_Listini NOT LIKE CONCAT('%', " & Session("Listino") & ", ';%'))) OR ((SessionID='" & SessionID & "') AND (articoli.SpedizioneGratis_Listini Is Not NULL) AND (SpedizioneGratis_Data_Fine < CURDATE())) ORDER BY id"
            Me.sdsArticoli_Spedizione_Gratis.SelectCommand = "SELECT vcarrello.*, articoli.SpedizioneGratis_Listini, articoli.SpedizioneGratis_Data_Inizio, articoli.SpedizioneGratis_Data_Fine FROM vcarrello LEFT OUTER JOIN articoli ON vcarrello.ArticoliId = articoli.id WHERE (SessionID='" & SessionID & "') AND (articoli.SpedizioneGratis_Listini Is Not NULL) AND (SpedizioneGratis_Listini LIKE CONCAT('%', " & Session("Listino") & ", ';%')) AND ((SpedizioneGratis_Data_Inizio <= CURDATE()) AND (SpedizioneGratis_Data_Fine >= CURDATE())) ORDER BY id"
        Else
            Me.sdsArticoli.SelectCommand = "SELECT vcarrello.*, articoli.SpedizioneGratis_Listini, articoli.SpedizioneGratis_Data_Inizio, articoli.SpedizioneGratis_Data_Fine FROM vcarrello LEFT OUTER JOIN articoli ON vcarrello.ArticoliId = articoli.id WHERE ((LoginId='" & LoginId & "') AND (articoli.SpedizioneGratis_Listini Is NULL)) OR ((LoginId='" & LoginId & "') AND (articoli.SpedizioneGratis_Listini Is Not NULL) AND (SpedizioneGratis_Listini NOT LIKE CONCAT('%', " & Session("Listino") & ", ';%'))) OR ((LoginId='" & LoginId & "') AND (articoli.SpedizioneGratis_Listini Is Not NULL) AND (SpedizioneGratis_Data_Fine < CURDATE())) ORDER BY id"
            Me.sdsArticoli_Spedizione_Gratis.SelectCommand = "SELECT vcarrello.*, articoli.SpedizioneGratis_Listini, articoli.SpedizioneGratis_Data_Inizio, articoli.SpedizioneGratis_Data_Fine FROM vcarrello LEFT OUTER JOIN articoli ON vcarrello.ArticoliId = articoli.id WHERE (LoginId='" & LoginId & "') AND (articoli.SpedizioneGratis_Listini Is Not NULL) AND (SpedizioneGratis_Listini LIKE CONCAT('%', " & Session("Listino") & ", ';%')) AND ((SpedizioneGratis_Data_Inizio <= CURDATE()) AND (SpedizioneGratis_Data_Fine >= CURDATE())) ORDER BY id"
        End If
    ...
    Protected Sub GridView1_PreRender(ByVal sender As Object, ByVal e As System.EventArgs) Handles GridView1.PreRender
        ...
        For i = 0 To GridView1.Rows.Count - 1
            ...
            tbQta = GridView1.Rows(i).FindControl("tbQta")
            img = GridView1.Rows(i).FindControl("imgDispo")
            dispo = GridView1.Rows(i).FindControl("lblDispo")
            arrivo = GridView1.Rows(i).FindControl("lblArrivo")
            importo = GridView1.Rows(i).FindControl("lblImporto")
            importoIvato = GridView1.Rows(i).FindControl("lblImportoIvato")
            peso = GridView1.Rows(i).FindControl("lblPeso")
    ...
    Private Sub SendOrder()
        ...
        Try
            ...
            If (GV_BuoniSconti.Rows.Count > 0) Then
                ...
                Me.Session("Ordine_DescrizioneBuonoSconto") = BuonoSconto_Descrizione1.Text & " " & BuonoSconto_Descrizione2.Text & " per un valore di € " & CDbl(BuonoSconto_Totale.Text.Replace("€ ", "")) & " Codice Applicato: " & BuonoSconto_Codice.Text
                Me.Session("Ordine_TotaleBuonoSconto") = CDbl(BuonoSconto_Totale.Text.Replace("€ ", ""))
    ...
    Public Sub LeggiPagamenti()
        ...
        lblIva.Text = "€ " & FormatNumber(calcola_iva(CDbl(lblSpeseSped.Text), Session("Iva_Vettori")), 2)
    ...
    Public Sub BindLstDestinazione()
        ...
        Try
            ...
            sqlString = "SELECT ID, CONCAT(RAGIONESOCIALEA, ' - ', NOMEA, ' - ',INDIRIZZOA, ', CAP: ', CAPA, ' - ',CITTAA,' (', PROVINCIAA, ')')AS CAMPO FROM utentiindirizzi where UTENTEID = " & Session("UTENTIID")
    ...
    Public Function getIndirizzoPrincipale() As String
        ...
        Try
            ...
            sqlString = "SELECT CONCAT(RAGIONESOCIALE, ' - ', COGNOMENOME, ' - ',INDIRIZZO, ', CAP: ', CAP, ' - ',CITTA,' (', PROVINCIA, ')')AS CAMPO FROM utenti where ID = " & Session("UTENTIID")
    ...
    Public Sub FillTableInfo()
        ...
        Try
            ...
            sqlString = "SELECT * FROM utenti where ID = " & Session("UTENTIID")
    ...
    Protected Sub btnSalvaDest_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnSalvaDest.Click
        ...
        Try
            ...
            If CHKPREDEFINITO.Checked = False Then
                ...
                sqlString &= " VALUES "
                sqlString &= " ("
                sqlString &= "  '" & Session("UTENTIID") & "', "
                sqlString &= "  '" & Me.tbRagioneSocialeA.Text.Replace("'", "''").ToUpper & "', "
                sqlString &= "  '" & Me.tbNomeA.Text.Replace("'", "''").ToUpper & "',"
                sqlString &= "  '" & Me.tbIndirizzo2.Text.Replace("'", "''").ToUpper & "', "
                sqlString &= "  '" & Me.tbCap2.Text.Replace("'", "''").ToUpper & "', "
                sqlString &= "  '" & Me.tbCitta2.Text.Replace("'", "''").ToUpper & "', "
                sqlString &= "  '" & Me.tbProvincia2.Text.Replace("'", "''").ToUpper & "', "
                sqlString &= "  '" & Me.tbNote.Text.Replace("'", "''").ToUpper & "', "
                sqlString &= "  '" & Me.tbZona.Text.Replace("'", "''").ToUpper & "'"

                cmd.CommandText = sqlString
                cmd.ExecuteNonQuery()
            Else
                'Se è stato selezionato come indirizzo secondario predefinito allora devo UPGRADE prima tutti i predefiniti a 0 e poi impostare l'unico predefinito a 1

                sqlString = "UPDATE utentiindirizzi SET Predefinito=0 WHERE UtenteId=" & Session("UTENTIID")
    ...
    Private Sub AggiornaDestinazionePredefinita(ByVal Aggiorna As Boolean)
        ...
        Try
            ...
            If Aggiorna = True Then
                ...
                sqlString = " UPDATE utentiindirizzi "
                sqlString &= " SET PREDEFINITO = 0 "
                sqlString &= " WHERE UTENTEID = " & Session("UTENTIID")
    ...
    Protected Sub LstDestinazione_PreRender(ByVal sender As Object, ByVal e As System.EventArgs) Handles LstDestinazione.PreRender
        ...
        compila_campi_destinazione_alternativa(LstDestinazione.SelectedValue)
    End Sub
    ...
    Protected Sub LstDestinazione_SelectedIndexChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles LstDestinazione.SelectedIndexChanged
        ...
        If LstDestinazione.SelectedItem.Value <> "0" Then
            Session("DESTINAZIONEALTERNATIVA") = LstDestinazione.SelectedItem.Value
        Else
            Session("DESTINAZIONEALTERNATIVA") = Nothing
        End If

        'SOLO PREDISPOSIZIONE DI CAMBIARE INDIRIZZO PREDEFINITO SULLA SCELTA:
        ...
        '    sqlString = "SELECT PREDEFINITO FROM utentiindirizzi where ID = " & LstDestinazione.SelectedItem.Value & " AND UTENTEID = " & Session("UTENTIID")

        '    cmd.Connection = conn
        '    cmd.CommandType = CommandType.Text
        '    cmd.CommandText = sqlString

        '    Dim Predefinito As String = ""

        '    If Not IsNothing(cmd.ExecuteScalar) And Not IsDBNull(cmd.ExecuteScalar) Then
        '        Predefinito = cmd.ExecuteScalar
        '    End If
    ...
    Function calcola_predefinito_destinazione_alternativa() As Integer
        Dim conn As New MySqlConnection
        Dim cmd As New MySqlCommand
        Dim sqlString As String = ""
        Dim dr As MySqlDataReader
        ...
        Try
            ...
            sqlString = "SELECT * FROM utentiindirizzi WHERE (UtenteId=" & Session("UtentiID") & ") AND (Predefinito=1)"
    ...
    Function compila_campi_destinazione_alternativa(ByVal idDestinazione As Integer) As Integer
        Dim conn As New MySqlConnection
        Dim cmd As New MySqlCommand
        Dim sqlString As String = ""
        Dim dr As MySqlDataReader
    ...
    Protected Sub gvArticoliGratis_PreRender(ByVal sender As Object, ByVal e As System.EventArgs) Handles gvArticoliGratis.PreRender
        ...
        For i = 0 To gvArticoliGratis.Rows.Count - 1
            ...
            tbQta = gvArticoliGratis.Rows(i).FindControl("tbQta")
            img = gvArticoliGratis.Rows(i).FindControl("imgDispo")
            dispo = gvArticoliGratis.Rows(i).FindControl("lblDispo")
            arrivo = gvArticoliGratis.Rows(i).FindControl("lblArrivo")
            importo = gvArticoliGratis.Rows(i).FindControl("lblImporto")
            importoIvato = gvArticoliGratis.Rows(i).FindControl("lblImportoIvato")
            peso = gvArticoliGratis.Rows(i).FindControl("lblPeso")
            ...
            If qta > 0 Then
                ...
                If IvaTipo = 1 Then
                    importo.Visible = True
                    importoIvato.Visible = False
                    gvArticoliGratis.Rows(i).FindControl("lblprezzo").Visible = True
                    gvArticoliGratis.Rows(i).FindControl("lblprezzoivato").Visible = False
					'Conteggio Totale per Buono Sconto
					TotaleMerce += CDbl(importo.Text)
                ElseIf IvaTipo = 2 Then
                    importo.Visible = False
                    importoIvato.Visible = True
                    gvArticoliGratis.Rows(i).FindControl("lblprezzo").Visible = False
                    gvArticoliGratis.Rows(i).FindControl("lblprezzoivato").Visible = True
					'Conteggio Totale per Buono Sconto
					TotaleMerce += CDbl(importoIvato.Text)
                End If
    ...
    Protected Sub Page_PreRenderComplete(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.PreRenderComplete
        ...
        If (Me.gvArticoliGratis.Rows.Count = 0) And (Me.GridView1.Rows.Count = 0) Then
        ...
        If (gvArticoliGratis.Rows.Count > 0) Or (GridView1.Rows.Count > 0) Then
        ...
        If GV_BuoniSconti.Rows.Count > 0 Then
            ...
            Else
                'lblBuonoSconto.Text = "€ " & FormatNumber(-CDbl(IIf(CDbl(scontoPercentuale.Text) > 0, (CDbl(lblTotale.Text.Replace("€ ", "")) / 100) * CDbl(scontoPercentuale.Text), CDbl(scontoFisso.Text))), 2)
				lblBuonoSconto.Text = "€ " & FormatNumber(-CDbl(IIf(CDbl(scontoPercentuale.Text) > 0, (CDbl(TotaleMerce) / 100) * CDbl(scontoPercentuale.Text), CDbl(scontoFisso.Text))), 2)
            End If

            totSconto.Text = IIf(CDbl(scontoPercentuale.Text) > 0, "Sconto in percentuale " & CDbl(scontoPercentuale.Text) & "%", IIf(Val(scontoVettore.Text) > 0, "SPEDIZIONE OMAGGIO", "Sconto fisso euro " & CDbl(scontoFisso.Text))) & "<br/>" & lblBuonoSconto.Text
        End If
    ...
    Sub Aggiorna_Prezzi_Carrello()
        ...
        If GridView1.Rows.Count > 0 Then
            For Each row In GridView1.Rows
                ...
                cmd.CommandText = "SELECT * FROM vsuperarticoli WHERE ID=" & ArtID.Text & " AND NListino=" & Listino & " GROUP BY offerteQntMinima, offerteMultipli, nlistino ORDER BY PrezzoPromo DESC"
                ...
                sb.Append(" Qnt = " & IIf(CLng(Qta.Text) <= 0, 0, CLng(Qta.Text)))
                ...
                sb.Append(", IdIvaRC = " & IdIvaRC)
                sb.Append(", ValoreIvaRC = " & ValoreIvaRC)
                sb.Append(", DescrizioneIvaRC = '" & DescrizioneIvaRC & "' ")
                sb.Append(", IdEsenzioneIva = " & IdEsenzioneIva)
                sb.Append(", ValoreEsenzioneIva = " & ValoreEsenzioneIva)
                sb.Append(", DescrizioneEsenzioneIva = '" & DescrizioneEsenzioneIva & "' ")
        ...
        If Me.gvArticoliGratis.Rows.Count > 0 Then
            For Each row In gvArticoliGratis.Rows
                ...
                sb.Append(", ValoreIvaRC = " & ValoreIvaRC)
                sb.Append(", DescrizioneIvaRC = '" & DescrizioneIvaRC & "' ")
                If (Session("IdEsenzioneIva") <> -1) Then
                    sb.Append(", IdEsenzioneIva = " & IdEsenzioneIva)
                    sb.Append(", ValoreEsenzioneIva = " & ValoreEsenzioneIva)
                    sb.Append(", DescrizioneEsenzioneIva = '" & DescrizioneEsenzioneIva & "' ")
                End If
    ...
    Protected Sub btSalvaPreventivo_Click(ByVal sender As Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles btSalvaPreventivo.Click
    ...
    Protected Sub ImageButton_DestinazioneAlternativa_Click(ByVal sender As Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles ImageButton_DestinazioneAlternativa.Click
        Me.PnlDestinazione.Visible = True
    End Sub

    Protected Sub ImageButton_InserisciNota_Click(ByVal sender As Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles ImageButton_InserisciNota.Click
        Me.Panel_Note.Visible = True
    End Sub
    ...
    Protected Sub btnModDest_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnModDest.Click
        ...
        Try
            ...
            If CHKPREDEFINITO.Checked = False Then
                ...
                sqlString &= "      RAGIONESOCIALEA= '" & Me.tbRagioneSocialeA.Text.Replace("'", "''").ToUpper & "', "
                sqlString &= "      NOMEA= '" & Me.tbNomeA.Text.Replace("'", "''").ToUpper & "', "
                sqlString &= "      INDIRIZZOA= '" & Me.tbIndirizzo2.Text.Replace("'", "''").ToUpper & "', "
                sqlString &= "      CAPA= '" & Me.tbCap2.Text.Replace("'", "''").ToUpper & "', "
                sqlString &= "      CITTAA= '" & Me.tbCitta2.Text.Replace("'", "''").ToUpper & "', "
                sqlString &= "      PROVINCIAA= '" & Me.tbProvincia2.Text.Replace("'", "''").ToUpper & "', "
                sqlString &= "      NOTE= '" & Me.tbNote.Text.Replace("'", "''").ToUpper & "', "
                sqlString &= "      ZONA= '" & Me.tbZona.Text.Replace("'", "''").ToUpper & "' "
                sqlString &= "  WHERE Id=" & LstDestinazione.SelectedValue
    ...
    Protected Sub btnElimDest_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnElimDest.Click
        ...
        Try
            ...
            If CHKPREDEFINITO.Checked = False Then
                sqlString = " DELETE FROM utentiindirizzi WHERE Id=" & LstDestinazione.SelectedValue

                cmd.CommandText = sqlString
                cmd.ExecuteNonQuery()
            Else
                'Nel Caso dell'eliminazione dopo aver cancellato il record, se quest'ultimo è il predefinito allora dopo la cancellazione imposto
                'l'ultimo indirizzo inserito come predefinito
                ...
                sqlString = "UPDATE utentiindirizzi SET Predefinito=1 WHERE UtenteId=" & Session("UTENTIID") & " ORDER BY Id DESC LIMIT 1"
    ...
    Protected Sub gvArticoliGratis_RowCommand(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewCommandEventArgs) Handles gvArticoliGratis.RowCommand
        If e.CommandName = "Aggiorna" Then
            btAggiorna_Click(sender, e)
        End If
    ...
    Protected Sub GridView1_RowDeleted(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewDeletedEventArgs) Handles GridView1.RowDeleted
        btAggiorna_Click(sender, Nothing)
    End Sub

    Protected Sub gvArticoliGratis_RowDeleted(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewDeletedEventArgs) Handles gvArticoliGratis.RowDeleted
        btAggiorna_Click(sender, Nothing)
    End Sub
    ...
    Protected Sub TB_BuonoSconto_TextChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles TB_BuonoSconto.TextChanged
        ...
        If TB_BuonoSconto.Text.Length > 0 Then
            For i = 0 To GridView1.Rows.Count - 1
                idArticolo = GridView1.Rows(i).FindControl("tbArtID")
                ...
                If controllaValiditaBuonoSconto(TB_BuonoSconto.Text, Session("AziendaID"), Val(idArticolo.Text), Session("UtentiId"), Session("UtentiTipoId"), Session("Listino")) = 1 Then
                    ...
                    If dr.HasRows Then
                        dr.Read()
                        Session("BuonoSconto_id") = dr.Item("idBuonoSconto")
    ...
    Function getUtilizzoBuonoSconto(ByVal codiceBuonoSconto As String, ByVal idAzienda As Integer) As Integer
        ...
        cmd.Connection = conn
        cmd.CommandType = CommandType.Text
        cmd.CommandText = "SELECT UtilizzaSoloUnaVolta FROM buoni_sconti WHERE (buonoSconto=@codiceBuono) AND (idAzienda=@idAzienda)"
        cmd.Parameters.AddWithValue("codiceBuono", codiceBuonoSconto)
        cmd.Parameters.AddWithValue("idAzienda", idAzienda)
    ...
    Function getBuonoScontoCodice(ByVal idBuonoSconto As Integer) As String
        ...
        cmd.Connection = conn
        cmd.CommandType = CommandType.Text
        cmd.CommandText = "SELECT buonoSconto FROM buoni_sconti WHERE idBuonoSconto=" & idBuonoSconto
    ...
    Protected Sub btInviaOrdine_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btInviaOrdine.Click
        ...
        Try
            ...
            If (controlla_articoli_quantita_zero() = 1) Then
                ...
                sqlString = "  SELECT UTENTI.AZIENDEID, "
                sqlString &= "        AZIENDE.RAGIONESOCIALE "
                sqlString &= " FROM UTENTI INNER JOIN AZIENDE "
                sqlString &= " ON UTENTI.AZIENDEID = AZIENDE.ID "
                sqlString &= " WHERE UTENTI.Id=" & Me.Session("UTENTIID")
                ...
                If drD.Read Then
                    ...
                    If drD.Item("AZIENDEID").ToString = "1" Then ' ENTROPIC
                        lblIntestDestinazione.Text = drD.Item("RAGIONESOCIALE")

                    ElseIf drD.Item("AZIENDEID").ToString = "2" Then ' WEBAFFARE
                        lblIntestDestinazione.Text = drD.Item("RAGIONESOCIALE")
                    End If
