...
Partial Class carrello
    ...
    Protected Sub Page_PreRender(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.PreRender
        ...
        End If
        Me.sdsArticoli.SelectCommand = Sqlstring & " WHERE (" & WhereUserId & " AND (articoli.SpedizioneGratis_Listini Is NULL)) OR (" & WhereUserId & " AND (articoli.SpedizioneGratis_Listini Is Not NULL) AND (SpedizioneGratis_Listini NOT LIKE CONCAT('%', " & Session("Listino") & ", ';%'))) OR (" & WhereUserId & " AND (articoli.SpedizioneGratis_Listini Is Not NULL) AND (SpedizioneGratis_Data_Fine < CURDATE())) ORDER BY id"
        Me.sdsArticoli_Spedizione_Gratis.SelectCommand = Sqlstring & " WHERE " & WhereUserId & " AND (articoli.SpedizioneGratis_Listini Is Not NULL) AND (SpedizioneGratis_Listini LIKE CONCAT('%', " & Session("Listino") & ", ';%')) AND ((SpedizioneGratis_Data_Inizio <= CURDATE()) AND (SpedizioneGratis_Data_Fine >= CURDATE())) ORDER BY id"
    ...
    Protected Sub Page_PreRenderComplete(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.PreRenderComplete
        ...
        lblTotale.Text = "€ " & FormatNumber(CDbl(lblImponibile.Text) + CDbl(lblIva.Text) + CDbl(lblSpeseAss.Text) + CDbl(lblSpeseSped.Text) + CDbl(lblPagamento.Text) + CDbl(lblBuonoSconto.Text), 2) '+ CDbl(lblBuonoScontoIVA.Text), 2)
    ...
    Public Function listaArticoliInCarrello() As String
        ...
        Dim articoloID As TextBox

        For i = 0 To GridView1.Rows.Count - 1
            articoloID = GridView1.Rows(i).FindControl("tbArtID")
            If stringa.Trim.Length = 0 Then
                stringa = articoloID.Text
            Else
                stringa += "," & articoloID.Text
            End If
        Next
    ...
    Public Function VerificaBuonoSconto(ByVal articoli As String, ByVal buonosconto As String, ByVal azienda As Integer, ByVal listino As String, ByVal utenteid As Integer, ByVal totaleMerceCarrello As Double) As Integer
        ...
        Dim listpar(4) As Object
        listpar(0) = buonosconto
        listpar(1) = azienda
        listpar(2) = listino
        listpar(3) = utenteid
        listpar(4) = CDbl(totaleMerceCarrello).ToString.Replace(",", ".")

        conn.ConnectionString = ConfigurationManager.ConnectionStrings("EntropicConnectionString").ConnectionString
        cmd.Connection = conn
        conn.Open()

        cmd.CommandText = String.Format("Select id from documenti where codicebuonosconto='{0}' and utentiid={1}", buonosconto, utenteid)
        ...
        cmd.CommandText = String.Format("Select sSql from Buoni_Sconti where buonosconto='{0}' and idAzienda={1} and ListaListini LIKE '%,{2},%' and (listautentiid=',' OR listautentiid LIKE '%,{3},%') and sogliaprezzo<={4} and curdate() between datainizio and datafine", listpar)

        tQuery = cmd.ExecuteScalar()

        If Not tQuery Is Nothing Then
            sQuery = String.Format("SELECT CASE WHEN COUNT(articoli.id)>0 THEN 1 ELSE 0 END as Trovato FROM articoli INNER JOIN ({0}) AS Test ON articoli.id=Test.id WHERE Test.id IN ({1})", tQuery, articoli)
    ...
    Function getBuonoScontoCodice(ByVal idBuonoSconto As Integer) As String
        ...
        cmd.Connection = conn
        cmd.CommandType = CommandType.Text
        cmd.CommandText = "SELECT buonoSconto FROM buoni_sconti WHERE id=" & idBuonoSconto
